...
## Corrections applied vs. the original hand‑drawn `ERD.png`

The earlier diagram had drifted from the real database. This version fixes:

1. **`gender_allowed` moved from STUDENT to HOSTEL** (it describes a hostel, not a student).
2. **Removed duplicate `name` on STUDENT**; added the missing **`email`** and **`password`**.
3. **Added `Hostel.name`** (now unique and referenced by `preferredHostel`).
4. **Connected ADMIN to HOSTEL** (previously ADMIN was disconnected).
5. **`preferredHostel` shown as a real relationship** to HOSTEL, not a loose attribute.
6. **Added missing fields**: `Allocation.active`, and `Complaint.category / slaViolated / escalatedAt / updatedAt` (the SLA/escalation feature).
7. Removed attributes not present in the database (`discipline_score`, `distance`) — see note below.
8. Consistent crow's‑foot notation (no mixing of FK‑as‑attribute with relationship diamonds).
